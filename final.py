...
# Set tema aplikasi
st.set_page_config(
    page_title="Aplikasi Menghitung Kalori Buah",
    page_icon="🍏",
    layout="wide",
    initial_sidebar_state="expanded",
)
# Gaya tambahan untuk tampilan aplikasi
st.markdown(
    """
    <style>
    /* Header */
...
st.markdown(
    ...
    <div style='background-color: #f0f0f0; padding: 20px; border-radius: 10px;'>
        <h2 style='color: black; text-align:center;'>Selamat Datang di Aplikasi Menghitung Kalori Buah</h2>
...
st.write("Pilih buah favorit Anda dan berapa beratnya, lalu kami akan memberi tahu Anda jumlah kalori yang terkandung.")
buah = st.selectbox('Pilih Buah', list(kalori_buah.keys()), format_func=lambda x: x.capitalize())

berat = st.text_input('Berat (gram)', '100')
# Bagian untuk menghitung kalori
...
if st.button('Hitung Kalori'):
    try:
        ...
        # Menampilkan hasil dengan pesan sukses
        st.success(f"Jumlah kalori dalam {berat} gram {buah} adalah: {kalori_total:.2f} kalori")
        ...
        # Menampilkan kandungan vitamin
        st.subheader("Kandungan Vitamin:")
...
kalori_target = st.sidebar.text_input('Masukkan Jumlah Kalori', '100')
kalori_target = float(kalori_target) if kalori_target else 100

st.sidebar.write('Buah-buahan dengan kalori serupa:')
for buah, info in kalori_buah.items():
    if abs((info['kalori'] / 100) * kalori_target - kalori_target) < 20:
